...
const findMovie = (moviesList, user) => {
    const vari = movies.find(movie => movie.userId === user)
    let title = vari.title
    return title
}

const listUsersWithMovies = (usersList, moviesList) => {
    let usersWhitMoviesArray = []
    for (user of usersList) {
        const { id, name, email } = user
        const movies = findMovie(moviesList, id)
        usersWhitMoviesArray.push({ name, email, movies })
    }
    return usersWhitMoviesArray
}
